...
public class QuizService {
    ...
    public void resetDemoQuizzes() {
        quizRepository.findQuizzes(Demo.COURSE_EXECUTION_ID).stream().skip(18).forEach(quiz -> {
            ...
            for (QuizQuestion quizQuestion : quiz.getQuizQuestions().stream()
                    .filter(quizQuestion -> quizQuestion.getQuestionAnswers().isEmpty()).collect(Collectors.toList())) {
