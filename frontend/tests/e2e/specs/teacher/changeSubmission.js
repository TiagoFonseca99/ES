  ...
  beforeEach(() => {
    ...
    cy.exec(
      'PGPASSWORD=' +
        ...
        ' -U ' +
        Cypress.env('USER') +
        ' -h localhost -c "WITH quest AS (INSERT INTO questions (title, content, status, course_id, key) VALUES (\'old\', \'old content\', \'SUBMITTED\', 2, 1200) RETURNING id)\nINSERT INTO submissions (question_id, user_id, course_execution_id) VALUES ((SELECT id from quest), 676, 11);" '
    );
    cy.exec(
      'PGPASSWORD=' +
        Cypress.env('PASS') +
  ...
  it('teacher change the submission', () => {
    ...
    cy.exec(
      'PGPASSWORD=' +
        ...
        ' -U ' +
        Cypress.env('USER') +
        ' -h localhost -c "WITH sub AS (SELECT question_id FROM submissions WHERE id IN (SELECT max(id) FROM submissions)) DELETE FROM options WHERE question_id IN (SELECT * FROM sub);\nWITH sub AS (DELETE FROM submissions WHERE id IN (SELECT max(id) FROM submissions) RETURNING question_id) DELETE FROM questions WHERE id IN (SELECT * FROM sub);\n"'
    );
  });
