describe('Teacher walkthrough', () => {
    ...
    it('login approves a submission', () => {
        ...
        cy.wait(3000);
        cy.exec('PGPASSWORD=fartodisto psql -d tutordb -U tomas -h localhost -c "With rev as (delete from reviews where user_id = 677 returning submission_id) delete from submissions where id = (select submission_id from rev); delete from questions where title = \'OIOI\'"');
