        ...
        <span class="headline">
          {{ 'Create Review' }}
            ...
            <v-card-actions>
                ...
                >Cancel</v-btn>
                <v-spacer />
                <v-btn color="blue darken-1" @click="createReview('REJECTED')">Reject</v-btn>
                <v-btn color="blue darken-1" @click="createReview('APPROVED')">Approve</v-btn>
            </v-card-actions>
        </v-card>
    </v-dialog>
</template>
...
export default class EditReview extends Vue {
    ...
    async createReview(status : string) {
        if (
            this.currentReview &&
            (!this.currentReview.justification)
        ) {
            await this.$store.dispatch(
                'error',
                'Review must have justification'
            );
            return;
        }
        try {
